...
echo " "
echo "See  https://github.com/node-red/linux-installers  for details."
echo "Thank you."
echo "**************************************************************************************************"
...
case $yn in
    [Yy]* )
        ...
        # if [ ! -d "/usr/lib/node_modules/node-red-node-serialport" ] && [ ! -d "$HOME/.node-red/node_modules/node-red-node-serialport" ]; then
            ...
            if [[ "$response" =~ ^([yY])+$ ]]; then
                EXTRANODES="node-red-node-pi-gpio node-red-node-random node-red-contrib-ibm-watson-iot node-red-node-ping node-red-contrib-play-audio node-red-node-smooth node-red-node-serialport"
                EXTRAW="install"
            fi
        # fi
...
sudo rm /usr/bin/update-nodejs-and-nodered
